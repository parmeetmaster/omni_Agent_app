...
class TransactionHistoryController extends GetxController implements GetxService{
  ...
  List<Transactions> _filterTransactionList  = [];
  List<Transactions> get transactionList => _transactionList;
  List<Transactions> get filterTransactionList => _filterTransactionList;
  ...
  List<int> _tabIndex = [0,1];

  List<int> get tabIndex => _tabIndex;
    ...
    if(!_offsetList.contains(offset)) {
      ...
      if(response.body['transactions'] != null && response.body['transactions'] != {} && response.statusCode==200){
        ...
        response.body['transactions'].forEach((transactionHistory) {
          ...
          _filterTransactionList = _transactionList;
        });
  ...
  void transactionSearch(String searchText) {
    if(searchText.isNotEmpty) {
      _filterTransactionList = [];
      _transactionList.forEach((transaction) {
        _filterTransactionList.addIf(transaction.userInfo.phone.contains(searchText) ||
            transaction.userInfo.name.toLowerCase().contains(searchText) || transaction.transactionId.toLowerCase().contains(searchText) , transaction);
      });
    }else {
      _filterTransactionList = transactionList;
    }
    update(['transaction_list']);
